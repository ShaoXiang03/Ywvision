"""
Crypto Markets Page - Raw Data Demo (Top 50, No Filtering)
"""
...
st.set_page_config(page_title="Crypto Markets - Raw Demo", page_icon="🪙")
st.title("🪙 Raw Polymarket Markets - Top 50 (Proof of API Access)")

# Session state for full load (optional)
if "load_all_raw" not in st.session_state:
    st.session_state.load_all_raw = False

col1, col2 = st.columns([1, 4])
with col1:
    if st.button("📥 Load All Raw Markets", use_container_width=True):
        st.session_state.load_all_raw = True
        st.rerun()

with col2:
    mode = "ALL raw markets" if st.session_state.load_all_raw else "first 50 raw markets"
...
with st.spinner("Fetching raw data from Gamma API..."):
    ...
    target_count = 1000 if st.session_state.load_all_raw else 50
